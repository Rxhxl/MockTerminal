...
public class Concatenate implements Command {
   ...
   */
  public Concatenate(FileSystem fileSystem) {
   ...
   */
  public void concatenate(List<String> files) {
    ...
    for (int i = 0; i < files.size(); i++) {
      if (getFinalFile(files.get(i)) != null) {
        ...
        invalidFileOutput = files.get(i) + " ";
      }
    }
    if (invalidFileName) {
      System.out.println("Sorry the file(s) " + invalidFileOutput.strip() + ","
          + " do not exist.");
    }
  }
  ...
  public File getFinalFile(String path) {
    ...
    } else currentDirectory = fileSystem.getCurrentDirectory();
    ...
    for (int i = 0; i < pathList.length; i++) {
      ...
      if (pathList[i].equals("."))continue;
        ...
        if (currentDirectory.getDirectory(pathList[i]) == null) {
          if (i == pathList.length - 1) break;
          else {
            System.out.println("Directory \"" + pathList[i] + "\" was not "
                + "found.");
            return null;
          }
        } else currentDirectory = currentDirectory.getDirectory(pathList[i]);
      }
    }
