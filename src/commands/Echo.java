...
public class Echo implements Command {
  ...
  public void run(List<String> tokens) {
    if(tokens.size() == 2) {
      ...
      echoAppendToFile(tokens.get(1).substring(1, tokens.get(1).length() - 1), tokens.get(3));
    } else if (tokens.get(2).contains(">")) {
      echoOverwriteToFile(tokens.get(1).substring(1, tokens.get(1).length() - 1), tokens.get(3));
    }
  }
   ...
   * specified by either a relative or absolute path
   * 
