...
public class MakeDirectory implements Command {
   ...
   * 
   * @param path String that is the path where the new directory is to be created
   */
  private void makeDirectory(String path) {
    ...
    if (currentDirectory.getDirectory(pathList[pathList.length - 1]) != null) {
      System.out.println("The directory \"" + pathList[pathList.length - 1] + "\" already exists.");
      return;
    }

    currentDirectory
        .addSubdirectory(new Directory(pathList[pathList.length - 1], currentDirectory));
  }
